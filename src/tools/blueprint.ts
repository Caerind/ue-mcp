...
export const blueprintTool: ToolDef = categoryTool(
  ...
  {
    read:              ro(bp("Read BP structure incl. SCS components. Params: assetPath", "read_blueprint", (p) => ({ path: p.assetPath }))),
    list_variables:    ro(bp("List variables. Params: assetPath", "list_blueprint_variables", (p) => ({ path: p.assetPath }))),
    list_functions:    ro(bp("List functions/graphs. Params: assetPath", "list_blueprint_functions", (p) => ({ path: p.assetPath }))),
    read_graph:        ro(bp("Read graph nodes. Params: assetPath, graphName", "read_blueprint_graph", (p) => ({ path: p.assetPath, graphName: p.graphName }))),
    read_graph_summary: ro(bp("Lightweight graph summary (nodes+edges only, ~10KB). Params: assetPath, graphName?", "read_blueprint_graph_summary", (p) => ({ path: p.assetPath, graphName: p.graphName }))),
    get_execution_flow: ro(bp("Trace exec pins from an entry point. Params: assetPath, graphName?, entryPoint?", "get_blueprint_execution_flow", (p) => ({ path: p.assetPath, graphName: p.graphName, entryPoint: p.entryPoint }))),
    get_dependencies:   ro(bp("Forward (classes/functions/assets) or reverse (referencers) deps. Params: assetPath, reverse?", "get_blueprint_dependencies", (p) => ({ path: p.assetPath, reverse: p.reverse }))),
    ...
    list_node_types:   ro(bp("List node types. Params: category?, includeFunctions?", "list_node_types")),
    search_node_types: ro(bp("Search nodes. Params: query", "search_node_types")),
    ...
    list_graphs:       ro(bp("List all graphs in a blueprint. Params: assetPath", "list_blueprint_graphs", (p) => ({ path: p.assetPath }))),
    ...
    list_local_variables: ro(bp("List local variables in a function. Params: assetPath, functionName", "list_local_variables")),
    validate:          ro(bp("Validate blueprint without saving (compile + collect diagnostics). Params: assetPath", "validate_blueprint")),
    read_component_properties: ro(bp("Dump ALL UPROPERTYs on a BP component template incl. array contents (#105). Params: assetPath, componentName", "read_component_properties", (p) => ({ path: p.assetPath, componentName: p.componentName }))),
    read_node_property: ro(bp("Read a node pin default OR a reflected node property for verification (#102). Params: assetPath, graphName?, nodeName, propertyName", "read_node_property", (p) => ({ path: p.assetPath, graphName: p.graphName, nodeName: p.nodeName, propertyName: p.propertyName }))),
    ...
    get_cdo_properties: ro(bp("Read UPROPERTY values from any C++ class CDO. Params: className, propertyNames? (#183)", "get_cdo_properties")),
