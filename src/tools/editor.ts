import { z } from "zod";
import { categoryTool, bp, ro, directive, type ToolDef, type ToolContext } from "../types.js";
...
export const editorTool: ToolDef = categoryTool(
  ...
  "Editor commands, Python execution, PIE, undo/redo, hot reload, viewport, performance, sequencer, build pipeline, logs, editor control.",
  {
    ...
    get_runtime_value: ro(bp("Read PIE actor value. Params: actorLabel, propertyName", "get_runtime_value")),
    get_pie_pawn: ro(bp("Resolve the controlled pawn in the active PIE world. Params: playerIndex? (default 0). Returns actorLabel/class/location/rotation (#228/#229)", "get_pie_pawn", (p) => ({ playerIndex: p.playerIndex }))),
    ...
    get_perf_stats: ro(bp("Editor performance stats", "get_editor_performance_stats")),
    ...
    get_viewport: ro(bp("Get viewport camera", "get_viewport_info")),
    ...
    get_sequence_info: ro(bp("Read sequence. Params: assetPath, includeSectionDetails? (attach sockets, first transform key values per track)", "get_sequence_info")),
    ...
    validate_assets: ro(bp("Run data validation. Params: directory?", "validate_assets")),
    get_build_status: ro(bp("Get build/map status", "get_build_status")),
    cook_content: bp("Cook content. Params: platform?", "cook_content"),
    get_log: ro(bp("Read output log. Params: maxLines?, filter?, category?", "get_output_log")),
    search_log: ro(bp("Search log. Params: query", "search_log")),
    get_message_log: ro(bp("Read message log. Params: logName?", "get_message_log")),
    list_crashes: ro(bp("List crash reports", "list_crashes")),
    get_crash_info: ro(bp("Get crash details. Params: crashFolder", "get_crash_info")),
    check_for_crashes: ro(bp("Check for recent crashes", "check_for_crashes")),
    ...
    get_dialog_policy: ro(bp("Get current dialog policies", "get_dialog_policy")),
    list_dialogs: ro(bp("List active modal dialogs", "list_dialogs")),
