...
export const gameplayTool: ToolDef = categoryTool(
  ...
  "Gameplay systems: physics, collision, navigation, input, behavior trees, AI (EQS, perception, State Trees, Smart Objects), game framework.",
  {
    ...
    get_navmesh_info:       ro(bp("Query nav system", "get_navmesh_info")),
    project_to_nav:         ro(bp("Project point to navmesh. Params: location, extent?", "project_point_to_navigation")),
    ...
    list_input_assets:      ro(bp("List input assets. Params: directory?, recursive?", "list_input_assets")),
    read_imc:               ro(bp("Read InputMappingContext mappings. Params: imcPath. Returns {key, inputAction, triggers[], modifiers[]}.", "read_imc")),
    list_input_mappings:    ro(bp("Alias for read_imc. List key→action bindings with triggers/modifiers. Params: imcPath", "list_imc_mappings")),
    ...
    list_behavior_trees:    ro(bp("List behavior trees. Params: directory?, recursive?", "list_behavior_trees")),
    get_behavior_tree_info: ro(bp("Inspect behavior tree (top-level + blackboard). Params: assetPath", "get_behavior_tree_info")),
    read_behavior_tree_graph: ro(bp("Walk BT tree: composites, tasks, decorators, services with blackboard keys. Params: assetPath", "read_behavior_tree_graph")),
    ...
    list_eqs_queries:       ro(bp("List EQS queries. Params: directory?", "list_eqs_queries")),
    ...
    list_state_trees:       ro(bp("List StateTrees. Params: directory?", "list_state_trees")),
    ...
    inspect_pie:            ro(bp("Inspect PIE runtime. Params: actorLabel?", "inspect_pie")),
    get_pie_anim_state:     ro(bp("Get PIE anim instance state. Params: actorLabel", "get_pie_anim_state")),
    get_pie_anim_properties: ro(bp("Read arbitrary UPROPERTY values on a PIE actor's AnimInstance (#139). Params: actorLabel, propertyNames? (omit = dump all)", "get_pie_anim_properties", (p) => ({ actorLabel: p.actorLabel, propertyNames: p.propertyNames }))),
    get_pie_subsystem_state: ro(bp("Read UPROPERTY values on a running subsystem in PIE (#139). Params: subsystemClass, scope? (game|world|engine|localplayer), propertyNames?", "get_pie_subsystem_state", (p) => ({ subsystemClass: p.subsystemClass, scope: p.scope, propertyNames: p.propertyNames }))),
    ...
    get_framework_info:     ro(bp("Get level framework classes", "get_game_framework_info")),
    get_navmesh_details:    ro(bp("Read RecastNavMesh generation params (cellSize, agentHeight, maxStepHeight, etc.) (#163)", "get_navmesh_details")),
